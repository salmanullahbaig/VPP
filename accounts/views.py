...
from affilate.create_wallet import create_wallet
...
class LogInView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        redirect_to = "index"#request.POST.get(REDIRECT_FIELD_NAME, request.GET.get(REDIRECT_FIELD_NAME))
        url_is_safe =False# is_safe_url(redirect_to, allowed_hosts=request.get_host(), require_https=request.is_secure())
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form ,*args, **kwargs):
        ...
        user.save()
        #print("****************************************refer saving...")
        registered_profile = Refer(user=user)
        registered_profile.recommended_by = User.objects.filter(username= 'superuser')[0]
        registered_profile.save()
        #print("****************************************refer saved")

        public_key,  private_key = create_wallet()
        wallet = user_wallet(user = user, public_key = public_key, secret_key =private_key )
        wallet.save()
...
def sign_up_ref(request, *args , **kwargs):
    """View function for home page of site."""
    code = str(kwargs.get('ref_code'))
    print("code is ", code)
    try:
        recommended_by_profile  = Refer.objects.get(code=code)
        print("refer by **",  recommended_by_profile.user)
    except:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            registered_profile = Refer(user=registered_user)
...
class Signup_ref(View):
    template_name = 'accounts/sign_up.html'
    form_class = SignUpForm
    @staticmethod
    def get(request, ref_code):
        print(ref_code)
        return super().get(request, *args, **kwargs)
        #super(View,self).get(request)
